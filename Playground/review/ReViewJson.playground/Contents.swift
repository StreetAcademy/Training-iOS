...
"updated_at": "2019-09-26T04:08:59Z",
"pushed_at": "2019-09-26T04:29:39Z",
...
"updated_at": "2019-09-25T23:12:49Z",
"pushed_at": "2019-09-25T23:12:47Z",
...
do {
        ...
        items.forEach { item in
            if let owner = item["owner"] as? [String: Any] {
                if let avatarUrl = owner["avatar_url"] as? String,
                    let siteAdmin = owner["site_admin"] as? Bool,
                    let url = owner["url"] as? String {
                    // avatar_urlをURL型に型変換
                    let avatarUrlCast = URL(string: avatarUrl)
                    avatarUrlCast
                    // site_adminの値をprint
                    print("siteAdmin：\(siteAdmin)")
                    // urlをURL型に型変換
                    let  urlCast = URL(string: url)
                    urlCast
                }
            }
        }
    ...
    if let items = array["items"] as? [[String: Any]] {
        items.forEach { item in
            if let license = item["license"] as? [String: Any] {
                if let name = license["name"] as? String {
                    print("name：\(name)")
                }
            }
        }
